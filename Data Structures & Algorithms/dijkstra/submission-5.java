class Solution {
    public Map<Integer, Integer> shortestPath(int n, List<List<Integer>> edges, int src) {
        ...
        // Dijkstra's: greedily finalizes the closest unvisited node each iteration.
        // This is only correct because edge weights are non-negative — if a later
        // path could have negative weight, it might undercut an already-finalized
        // distance, which greedy could never revise.
        // adjList.get()[0] -> v
        // adjList.get()[1] -> w
        Map<Integer, List<int[]>> adjList = toAdjList(edges, n);
        // let us assume the constraint is a dense graph (E ~ V^2)
        // so we use array-based dijkstra
        // distance[i] holds the current best known distance from src to i.
        // Before i is finalized, this value can still improve via relaxation;
        // once i is finalized (added to shortestPaths), it's the true shortest distance.
